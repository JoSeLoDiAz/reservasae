...
export function BarraDeGestion({
      ...
      await alGuardar(async () => {
        ...
        if (cambioAccion && accion?.ofertaId) {
          ...
          if (accion.disponibles === 0 && accion.ofertaId !== lead.oferta?.id) {
            const escrito = window.prompt(
              /// Idioma de venta: el servicio se queda sin
              /// disponibilidad, no «sin cupos». La pregunta y el
              /// registro a su nombre siguen iguales.
              `«${accion.etiqueta}»: sin disponibilidad del servicio. ` +
                "¿Por qué se asigna por encima de lo disponible? Queda registrado a su nombre.",
            );
            if (!escrito?.trim()) return;
            motivo = escrito.trim();
          }
  ...
  /// va a dar.
  ///
  /// En idioma de VENTA, no de cupos: Grupo AE vende servicios del
  /// portafolio (licencias Google, Chromebooks, soporte, cursos) y
  /// «no quedan cupos» o «sin oferta asignada» eran el idioma del
  /// CRM de formación del que esto se heredó. Las claves se quedan
  /// como están porque son el código que manda el backend.
  const MOTIVO_CORTO: Record<string, string> = {
    OFERTA_CERRADA: "El servicio ya no se oferta",
    LLENO: "Sin disponibilidad del servicio",
    SIN_OFERTA: "Falta elegir el servicio del portafolio",
  };
          ...
          <Desplegable
            ...
            opciones={[
              ...
              ...opciones.acciones.map((a) => ({
                ...
                detalle: a.cubre
                  ? `${a.ubicacion ?? ""} · ${a.disponibles} disponibles`
