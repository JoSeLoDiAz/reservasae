...
/// Esta instancia puede abrir el panel por el túnel.
///
/// Apagado por defecto, y no es una formalidad: la puerta de
/// arriba existe porque el panel lleva nombres, documentos,
/// correos y celulares, y un enlace de túnel lo tiene cualquiera
/// a quien se lo reenvíen.
///
/// Se enciende SOLO mientras `grupoae_prueba` esté vacía o con
/// datos inventados, que es lo que hace falta para poder enseñar
/// el CRM por el enlace. El día que esa base lleve inscritos de
/// verdad, se quita la variable de `scripts/dev-ae.ps1` y el
/// corte vuelve a cerrarse solo.
///
/// El panel sigue pidiendo sesión en los dos casos: esto es la
/// segunda vuelta de llave, nunca la única.
...
export function middleware(peticion: NextRequest) {
  ...
  if (!esTunel(peticion) || PANEL_POR_TUNEL) return NextResponse.next();
