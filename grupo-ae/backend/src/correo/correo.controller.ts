...
export class CorreoController {
  ...
  async estado() {
    ...
    return {
      ...
      nombre: process.env.SMTP_NOMBRE ?? 'CRM Grupo AE',
  ...
  async probar(@Body() dto: ProbarCorreoDto, @AdminActual() admin: Admin) {
    ...
    const r = await this.correo.enviar({
      para: dto.para,
      asunto: 'CRM Grupo AE · prueba de correo',
      texto:
        `Si está leyendo esto, el correo del CRM de Grupo AE quedó funcionando.\n\n` +
        `Lo mandó ${admin.nombre} desde el panel. No hay que contestarlo.\n`,
      html:
        '<p>Si está leyendo esto, el correo del <strong>CRM de Grupo AE</strong> quedó ' +
        'funcionando.</p>' +
